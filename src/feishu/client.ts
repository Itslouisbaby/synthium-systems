...
function resolveAppSecret(config?: { appSecret?: string; appSecretFile?: string }): string | undefined {
...
export function getFeishuClient(accountIdOrAppId?: string, explicitAppSecret?: string) {
  ...
  if (!appId && feishuCfg?.appId) appId = feishuCfg.appId.trim();
  if (!appSecret) appSecret = resolveAppSecret(feishuCfg);
  if (!domain) domain = feishuCfg?.domain;

  // Environment variables fallback
  if (!appId) appId = process.env.FEISHU_APP_ID?.trim();
  if (!appSecret) appSecret = process.env.FEISHU_APP_SECRET?.trim();
