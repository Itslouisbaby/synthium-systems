...
This document explains how Synth AI manages sessions end-to-end:
...
Synth AI is designed around a single **Gateway process** that owns session state.
...
Synth AI persists sessions in two layers:
...
Synth AI resolves these via `src/config/sessions.ts`.
...
Synth AI intentionally does **not** “fix up” transcripts; the Gateway uses `SessionManager` to read/write them.
...
These are Pi runtime semantics (Synth AI consumes the events, but Pi decides when to compact).
...
Synth AI also enforces a safety floor for embedded runs:

- If `compaction.reserveTokens < reserveTokensFloor`, Synth AI bumps it.
- Default floor is `20000` tokens.
- Set `agents.defaults.compaction.reserveTokensFloor: 0` to disable the floor.
- If it’s already higher, Synth AI leaves it alone.
...
Synth AI supports “silent” turns for background tasks where the user should not see intermediate output.

Convention:

- The assistant starts its output with `NO_REPLY` to indicate “do not deliver a reply to the user”.
- Synth AI strips/suppresses this in the delivery layer.

As of `2026.1.10`, Synth AI also suppresses **draft/typing streaming** when a partial chunk begins with `NO_REPLY`, so silent operations don’t leak partial output mid-turn.
...
Synth AI uses the **pre-threshold flush** approach:
...
Pi also exposes a `session_before_compact` hook in the extension API, but Synth AI’s
