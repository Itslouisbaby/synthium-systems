...
Synth AI integrates external CLIs via JSON-RPC. Two patterns are used today.
...
- Synth AI owns lifecycle when `channels.signal.autoStart=true`.
...
- Synth AI spawns `imsg rpc` as a child process (legacy iMessage integration).
