...
Synth AI will offer the local install path automatically.
...
3. Configure Synth AI with those credentials.
...
- Talk to Synth AI via Teams DMs, group chats, or channels.
...
- On startup, Synth AI resolves team/channel and user allowlist names to IDs (when Graph permissions allow)
...
Before configuring Synth AI, you need to create an Azure Bot resource.
...
```json
{
  ...
  "description": { "short": "Synth AI in Teams", "full": "Synth AI in Teams" },
...
Synth AI handles this by returning quickly and sending replies proactively, but very slow responses may still cause issues.
...
By default, Synth AI only downloads media from Microsoft/Teams hostnames. Override with `channels.msteams.mediaAllowHosts` (use `["*"]` to allow any host).
...
Uploaded files are stored in a `/Synth AIShared/` folder in the configured SharePoint site's default document library.

## Polls (Adaptive Cards)

Synth AI sends Teams polls as Adaptive Cards (there is no native Teams poll API).
