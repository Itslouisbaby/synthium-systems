...
Matrix is an open, decentralized messaging protocol. Synth AI connects as a Matrix **user**
...
3. Get an access token for the bot account:
   ...
   - Or set `channels.matrix.userId` + `channels.matrix.password`: Synth AI calls the same
...
- On first connection, Synth AI requests device verification from your other sessions.
- Verify the device in another Matrix client (Element, etc.) to enable key sharing.
- If the crypto module cannot be loaded, E2EE is disabled and encrypted rooms will not decrypt;
  Synth AI logs a warning.
...
- On startup, Synth AI resolves room/user names in allowlists to IDs and logs the mapping; unresolved entries are ignored for allowlist matching.
