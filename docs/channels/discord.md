...
- Talk to Synth AI via Discord DMs or guild channels.
...
3. Configure Synth AI with `channels.discord.token` (or `DISCORD_BOT_TOKEN` as a fallback).
...
This is the “Discord Developer Portal” setup for running Synth AI in a server (guild) channel like `#help`.
...
### 2) Enable the gateway intents Synth AI needs
...
Discord uses numeric ids everywhere; Synth AI config prefers ids.
...
- Owner hint: when a per-guild or per-channel `users` allowlist matches the sender, Synth AI treats that sender as the owner in the system prompt. For a global owner across channels, set `commands.ownerAllowFrom`.
...
When enabled, Synth AI uses the member identity for allowlists and labels the
...
- On startup, Synth AI resolves channel/user names in allowlists to IDs (when the bot can search members)
  and logs the mapping; unresolved entries are kept as typed.

Native command notes:

- The registered commands mirror Synth AI’s chat commands.
...
- Slash commands may still be visible in Discord UI to users who aren’t allowlisted; Synth AI enforces allowlists on execution and replies “not authorized”.
