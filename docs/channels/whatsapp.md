...
WhatsApp requires a real mobile number for verification. VoIP and virtual numbers are usually blocked. There are two supported ways to run Synth AI on WhatsApp:

### Dedicated number (recommended)

Use a **separate phone number** for Synth AI. Best UX, clean routing, no self-chat quirks. Ideal setup: **spare/old Android phone + eSIM**. Leave it on Wi‑Fi and power, and link it via QR.

**WhatsApp Business:** You can use WhatsApp Business on the same device with a different number. Great for keeping your personal WhatsApp separate — install WhatsApp Business and register the Synth AI number there.
...
Quick fallback: run Synth AI on **your own number**. Message yourself (WhatsApp “Message yourself”) for testing so you don’t spam contacts. Expect to read verification codes on your main phone during setup and experiments. **Must enable self-chat mode.**
...
- Early Synth AI builds supported Twilio’s WhatsApp Business integration.
...
If you run Synth AI on your **personal WhatsApp number**, enable `channels.whatsapp.selfChatMode` (see sample above).
...
**Will Synth AI message random contacts when I link WhatsApp?**  
No. Default DM policy is **pairing**, so unknown senders only get a pairing code and their message is **not processed**. Synth AI only replies to chats it receives, or to sends you explicitly trigger (agent/CLI).
...
**Can multiple people use different Synth AI instances on one WhatsApp number?**  
...
- Best results: OGG/Opus. Synth AI rewrites `audio/ogg` to `audio/ogg; codecs=opus`.
