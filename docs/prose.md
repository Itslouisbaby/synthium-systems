---
summary: "OpenProse: .prose workflows, slash commands, and state in Synth AI"
read_when:
...
OpenProse is a portable, markdown-first workflow format for orchestrating AI sessions. In Synth AI it ships as a plugin that installs an OpenProse skill pack plus a `/prose` slash command. Programs live in `.prose` files and can spawn multiple sub-agents with explicit control flow.
...
OpenProse registers `/prose` as a user-invocable skill command. It routes to the OpenProse VM instructions and uses Synth AI tools under the hood.
...
## Synth AI runtime mapping

OpenProse programs map to Synth AI primitives:

| OpenProse concept         | Synth AI tool    |
| ------------------------- | ---------------- |
...
Treat `.prose` files like code. Review before running. Use Synth AI tool allowlists and approval gates to control side effects.
