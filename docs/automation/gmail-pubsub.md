---
summary: "Gmail Pub/Sub push wired into Synth AI webhooks via gogcli"
read_when:
  - Wiring Gmail inbox triggers to Synth AI
  - Setting up Pub/Sub push for agent wake
title: "Gmail PubSub"
---

# Gmail Pub/Sub -> Synth AI

Goal: Gmail watch -> Pub/Sub push -> `gog gmail watch serve` -> Synth AI webhook.

## Prereqs

- `gcloud` installed and logged in ([install guide](https://docs.cloud.google.com/sdk/docs/install-sdk)).
- `gog` (gogcli) installed and authorized for the Gmail account ([gogcli.sh](https://gogcli.sh/)).
- Synth AI hooks enabled (see [Webhooks](/automation/webhook)).
...
Use the Synth AI helper to wire everything together (installs deps on macOS via brew):
...
Path note: when `tailscale.mode` is enabled, Synth AI automatically sets
...
- `--hook-url` points to Synth AI `/hooks/gmail` (mapped; isolated run + summary to main).
- `--include-body` and `--max-bytes` control the body snippet sent to Synth AI.
