---
summary: "Synth AI CLI reference for `openclaw` commands, subcommands, and options"
read_when:
...
Synth AI uses a lobster palette for CLI output.
...
Synth AI can surface provider usage/quota when OAuth/API creds are available.
...
- `gateway status` also surfaces legacy or extra gateway services when it can detect them (`--deep` adds system-level scans). Profile-named Synth AI services are treated as first-class and aren't flagged as "extra".
