...
read_when:
  - You want to understand what “context” means in Synth AI
...
“Context” is **everything Synth AI sends to the model for a run**. It is bounded by the model’s **context window** (token limit).

Beginner mental model:

- **System prompt** (Synth AI-built): rules, tools, skills list, time/runtime, and injected workspace files.
...
## How Synth AI builds the system prompt

The system prompt is **Synth AI-owned** and rebuilt each run. It includes:
...
By default, Synth AI injects a fixed set of workspace files (if present):
