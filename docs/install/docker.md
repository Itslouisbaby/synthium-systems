---
summary: "Optional Docker-based setup and onboarding for Synth AI"
read_when:
...
- **Yes**: you want an isolated, throwaway gateway environment or to run Synth AI on a host without local installs.
- **No**: you’re running on your own machine and just want the fastest dev loop. Use the normal install flow instead.
- **Sandboxing note**: agent sandboxing uses Docker too, but it does **not** require the full gateway to run in Docker. See [Sandboxing](/gateway/sandboxing).

This guide covers:

- Containerized Gateway (full Synth AI in Docker)
...
- `user` must be root for `apt-get` (omit `user` or set `user: "0:0"`).
  Synth AI auto-recreates containers when `setupCommand` (or docker config) changes
...
- Custom tools not found: Synth AI runs commands with `sh -lc` (login shell), which
