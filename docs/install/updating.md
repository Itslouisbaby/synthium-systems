---
summary: "Updating Synth AI safely (global install or source), plus rollback strategy"
read_when:
  - Updating Synth AI
...
Synth AI is moving fast (pre “1.0”). Treat updates like shipping infra: update → run checks → restart (or use `openclaw update`, which restarts) → verify.
...
- Detect and migrate older gateway services (launchd/systemd; legacy schtasks) to current Synth AI services.
