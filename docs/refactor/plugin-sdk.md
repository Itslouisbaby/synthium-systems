...
Accessed via `Synth AIPluginApi.runtime` so plugins never import `src/**`.
...
export type PluginRuntime = {
  channel: {
    text: {
      ...
      resolveTextChunkLimit(cfg: Synth AIConfig, channel: string, accountId?: string): number;
      hasControlCommand(text: string, cfg: Synth AIConfig): boolean;
    };
    reply: {
    ...
    mentions: {
      buildMentionRegexes(cfg: Synth AIConfig, agentId?: string): RegExp[];
    ...
    debounce: {
      ...
      resolveInboundDebounceMs(cfg: Synth AIConfig, channel: string): number;
    };
    commands: {
  ...
  state: {
    resolveStateDir(cfg: Synth AIConfig): string;
  };
};
```
...
- Add `api.runtime` to `Synth AIPluginApi` with the surface above.
