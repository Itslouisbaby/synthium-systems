...
- inject build metadata into Info.plist: `Synth AIBuildTimestamp` (UTC) and `Synth AIGitCommit` (short hash) so the About pane can show build, git, and debug/release channel.
...
- `Synth AIBuildTimestamp`: ISO8601 UTC at package time
- `Synth AIGitCommit`: short git hash (or `unknown` if unavailable)
...
TCC permissions are tied to the bundle identifier _and_ code signature. Unsigned debug builds with changing UUIDs were causing macOS to forget grants after each rebuild. Signing the binaries (ad‑hoc by default) and keeping a fixed bundle id/path (`dist/Synth AI.app`) preserves the grants between builds, matching the VibeTunnel approach.
