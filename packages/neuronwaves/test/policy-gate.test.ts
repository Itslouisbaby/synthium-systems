...
function baseConfig(level: 1 | 2 | 3): AutonomyConfig {
  return {
    ...
    limits: { maxActionsPerRun: 10, maxToolCallsPerRun: 50 },
  };
}
...
describe("policy gate", () => {
  it("Level 1 allows local_write and blocks external_read", () => {
    ...
    const allow = decidePolicy({ autonomy: cfg, actionClass: "local_write" });
  ...
  it("Level 2 allows external_read", () => {
    ...
    const res = decidePolicy({ autonomy: cfg, actionClass: "external_read", toolName: "web_search" });
  ...
  it("Level 2 blocks irreversible writes", () => {
    const cfg = baseConfig(2);
    const res = decidePolicy({ autonomy: cfg, actionClass: "external_write_irreversible" });
  ...
  it("Level 3 allows external_read and reversible writes but blocks money movement", () => {
    ...
    const blockMoney = decidePolicy({ autonomy: cfg, actionClass: "money_movement" });
